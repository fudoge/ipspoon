...
#ifndef IPSPOON_VERSION
#define IPSPOON_VERSION "dev"
#endif
...
void iptohostname(int af, char *input_addr) {
    ...
    char hbuf[BUFLEN];
    ret = getnameinfo(req_addr, addr_len, hbuf, sizeof(hbuf), NULL, 0,
                      NI_NAMEREQD);
    if (ret != 0) {
...
int main(int argc, char *argv[]) {
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        printf("%s %s\n", argv[0], IPSPOON_VERSION);
        return 0;
    }
    if (argc != 3) {
        fprintf(
            stderr,
            "Usage: %s <option> <address or hostname>\n\t-4 <address>: get "
            "hostname from ipv4 address\n\t-6 <address>: get hostname from "
